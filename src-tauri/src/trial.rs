...
// Obfuscated key fragments — concatenated at runtime to form the HMAC base secret.
// Split across multiple constants so `strings` on the binary won't reveal a single key.
const K0: [u8; 8] = [0x41, 0x4c, 0x41, 0x4e, 0x5f, 0x45, 0x43, 0x48]; // ALAN_ECH
const K1: [u8; 8] = [0x4f, 0x5f, 0x54, 0x52, 0x49, 0x41, 0x4c, 0x5f]; // O_TRIAL_
const K2: [u8; 8] = [0x53, 0x49, 0x47, 0x4e, 0x5f, 0x32, 0x30, 0x32]; // SIGN_202
const K3: [u8; 8] = [0x36, 0x5f, 0x76, 0x31, 0x2e, 0x33, 0x2e, 0x30]; // 6_v1.3.0
